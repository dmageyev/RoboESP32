...
void executePhysicalMovement(char dir, int speed, int balance) {
  // Діагностика блокування рушія паркінгом
  if (parkingBrake && (dir != 'S' || speed != 0)) {
    Serial.println("[MOTOR][BLOCKED] Спроба руху відхилена: активне паркувальне гальмо!");
    dir = 'S';
    speed = 0;
  }
  ...
  if (balance > 0) {
    rightSpeed = speed * (50 - balance) / 50;
    if (rightSpeed < 0) rightSpeed = 0;
  } else if (balance < 0) {
    leftSpeed = speed * (50 + balance) / 50;
    if (leftSpeed < 0) leftSpeed = 0;
  }

  // --- Автоматика стоп-сигналу та заднього ходу + діагностика ---
  if (dir == 'S' || parkingBrake) {
    if (digitalRead(PIN_STOP) == LOW) {
      digitalWrite(PIN_STOP, HIGH);
      Serial.println("[LIGHT][AUTO] Машинка зупинена -> Автоматично увімкнено СТОП-СИГНАЛ");
    }
    ...
    if (digitalRead(PIN_REVERSE) == LOW) {
      ...
      Serial.println("[LIGHT][AUTO] Рух назад -> Автоматично увімкнено СВІТЛО ЗАДНЬОГО ХОДУ");
    }
  } else {
    if (digitalRead(PIN_STOP) == HIGH || digitalRead(PIN_REVERSE) == HIGH) {
      Serial.println("[LIGHT][AUTO] Рух вперед/поворот -> Автоматичні ліхтарі вимкнено");
    }
    digitalWrite(PIN_STOP, LOW);       
    digitalWrite(PIN_REVERSE, LOW);
  }

  // Лог фізичних значень ШІМ на драйвер моторів
  Serial.printf("[MOTOR] Напрямок: %c | ШІМ Лівий: %d | ШІМ Правий: %d\n", dir, leftSpeed, rightSpeed);

  switch (dir) {
    case 'F':
      ledcWrite(0, leftSpeed);  ledcWrite(1, 0);
      ledcWrite(2, rightSpeed); ledcWrite(3, 0);
      break;
    case 'B':
      ledcWrite(0, 0);          ledcWrite(1, leftSpeed);
      ledcWrite(2, 0);          ledcWrite(3, rightSpeed);
      break;
    case 'L':
      ledcWrite(0, 0);          ledcWrite(1, leftSpeed);
      ledcWrite(2, rightSpeed); ledcWrite(3, 0);
...
void loop() {
  ...
  // Асинхронне блимання ліхтарями
  if (hazardActive || turnLeftActive || turnRightActive) {
    if (millis() - lastBlinkTime >= 400) {
      lastBlinkTime = millis();
      blinkState = !blinkState;
      
      if (hazardActive) {
        digitalWrite(PIN_TURN_L, blinkState ? HIGH : LOW);
        digitalWrite(PIN_TURN_R, blinkState ? HIGH : LOW);
      } else {
        digitalWrite(PIN_TURN_L, turnLeftActive ? (blinkState ? HIGH : LOW) : LOW);
        digitalWrite(PIN_TURN_R, turnRightActive ? (blinkState ? HIGH : LOW) : LOW);
      }
    }
  }
}
